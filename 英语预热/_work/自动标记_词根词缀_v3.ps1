# Deterministic root/affix tagging v3 (v2 + contains>=4 + ity/-y guard)
...
function Root-Hit([string]$stem) {
  foreach ($rt in $rootTok) {
    ...
    if ($rt.len -ge 4 -and $stem.Contains($rt.tok)) { return $rt.fam }
  }
  return $null
}
    ...
    if ($wl -match '^[a-z][a-z .''\-]*$' -and $wl.Length -ge 3) {
      ...
      foreach ($st in ($sufTok | Sort-Object { -$_.len })) {
        if ($workStr.Length -gt $st.len -and $workStr.EndsWith($st.tok)) {
          if ($st.tok -eq 'y' -and $workStr.EndsWith('ity')) { continue }
...
Write-Output ('v3 done tagged=' + $statTagged + ' plain=' + $statPlain)
