# Deterministic root/affix tagging v1 - rules parsed from spec.md tables.
...
# token lists
$preTok = New-Object System.Collections.Generic.List[object]   # {tok,len,fam}
...
function Root-Hit([string]$stem) {
  foreach ($rt in $rootTok) {
    ...
    if ($stem.Length -le $rt.len + 1 -and $stem.Contains($rt.tok)) { return $rt.fam }
...
$statTagged = 0; $statPlain = 0; $statProp = 0
$tagCount = @{}
for ($s = 0; $s -lt $bounds.Count; $s++) {
  ...
  for ($i = $a; $i -le $b; $i++) {
    ...
    $pre = @(); $root = @(); $suf = @()
    if ($wl -match '^[a-z][a-z .''\-]*$' -and $wl.Length -ge 2) {
      $isProp = ($w -cmatch '^[A-Z][A-Za-z]+$' -or $w -cmatch '^[A-Z]{2,}$')
      if ($isProp) { $statProp++ }
      else {
        $workStr = $wl
        # suffix first
        ...
        foreach ($pt in ($preTok | Sort-Object { -$_.len })) {
          if ($workStr.Length -gt $pt.len -and $workStr.StartsWith($pt.tok)) {
            ...
            if ($rem.Length -ge 2 -and ($heads.Contains($rem) -or $null -ne (Root-Hit $rem))) {
    ...
    foreach ($t in ($pre + $root + $suf)) { if ($tagCount.ContainsKey($t)) { $tagCount[$t]++ } else { $tagCount[$t] = 1 } }
...
Write-Output ('done tagged=' + $statTagged + ' plain=' + $statPlain + ' prop=' + $statProp)
Write-Output ('tags used: ' + $tagCount.Count)
($tagCount.GetEnumerator() | Sort-Object Value -Descending | Select-Object -First 25) | ForEach-Object { Write-Output ($_.Key + ' = ' + $_.Value) }
